<?php
/*******************************************************
 * ⚙️ GOFAST — SOLICITAR ENVÍO INTERMUNICIPAL PARA ADMIN
 * Shortcode: [gofast_admin_solicitar_intermunicipal]
 * URL: /admin-solicitar-intermunicipal
 * 
 * Página de confirmación y solicitud del servicio intermunicipal (admin)
 * Asigna el servicio al mensajero seleccionado
 *******************************************************/

function gofast_admin_solicitar_intermunicipal_shortcode() {
    global $wpdb;
    ...
    // Verificar rol
    if ($rol !== 'admin') {
        $usuario_check = $wpdb->get_row($wpdb->prepare(
            "SELECT rol FROM usuarios_gofast WHERE id = %d AND activo = 1",
            $user_id
        ));
        if (!$usuario_check || strtolower($usuario_check->rol) !== 'admin') {
    ...
    $cotizacion = isset($_SESSION['gofast_admin_intermunicipal']) ? $_SESSION['gofast_admin_intermunicipal'] : null;
    
    if (!$cotizacion || empty($cotizacion['destino']) || empty($cotizacion['mensajero_id'])) {
        // Redirigir al cotizador si no hay datos
        $url_cotizar = esc_url(home_url('/admin-cotizar-intermunicipal'));
        return '<script>window.location.href = "'.$url_cotizar.'";</script>';
    }

    $mensajero_id = isset($cotizacion['mensajero_id']) ? intval($cotizacion['mensajero_id']) : 0;
    $mensajero_nombre = isset($cotizacion['mensajero_nombre']) ? $cotizacion['mensajero_nombre'] : '';
    ...
    if (!$mensajero_seleccionado) {
        unset($_SESSION['gofast_admin_intermunicipal']);
        return '<div class="gofast-box">Error: El mensajero seleccionado no es válido. <a href="'.esc_url(home_url('/admin-cotizar-intermunicipal')).'">Volver al cotizador</a></div>';
    }
    ...
    if (!isset($destinos_intermunicipales[$destino])) {
        unset($_SESSION['gofast_admin_intermunicipal']);
        return '<div class="gofast-box">Error: Destino no válido. <a href="'.esc_url(home_url('/admin-cotizar-intermunicipal')).'">Volver al cotizador</a></div>';
    }
    ...
    if (!empty($_POST['confirmar_intermunicipal'])) {
        ...
        // Actualizar dirección de recogida en la sesión si se proporcionó
        if (!empty($direccion_recogida_nueva)) {
            $_SESSION['gofast_admin_intermunicipal']['direccion_recogida'] = $direccion_recogida_nueva;
        }
        ...
        if (empty($nombre) || empty($telefono) || empty($direccion_destino)) {
        ...
        } elseif (empty($destino) || $valor_envio <= 0 || empty($mensajero_id)) {
            ...
            // Asignar al mensajero seleccionado por el admin
            $mensajero_id_servicio = $mensajero_id;
            ...
            if (!empty($column_exists)) {
                $data_insert['asignado_por_user_id'] = $usuario ? $usuario->id : null;
            }
            ...
            if ($insertado === false) {
                ...
                if (!$service_id || $service_id <= 0) {
                    ...
                    // Admin: NO redirigir a servicio-registrado, solo mostrar mensaje
                    ...
                    $success_message .= '<p style="margin-bottom:0;"><a href="' . esc_url(home_url('/mis-pedidos')) . '" class="gofast-btn" style="display:inline-block;margin-top:12px;">Ver pedidos</a></p>';
                    $success_message .= '</div>';
    ...
    if (empty($destino) || $valor_envio <= 0 || empty($mensajero_id)) {
        unset($_SESSION['gofast_admin_intermunicipal']);
        return '<div class="gofast-box">Error: Datos de cotización incompletos. <a href="'.esc_url(home_url('/admin-cotizar-intermunicipal')).'">Volver al cotizador</a></div>';
    }
    ...
    <div class="gofast-checkout-wrapper">
        ...
        <div class="gofast-box">
            ...
            <div class="gofast-box" style="background: #e3f2fd; border-left: 4px solid #2196F3; padding: 12px; margin-bottom: 16px;">
                <strong>⚙️ Mensajero asignado:</strong> <?php echo esc_html($mensajero_nombre); ?>
            </div>
        ...
        <div class="gofast-box">
            ...
            <form method="post" action="<?php echo esc_url($_SERVER['REQUEST_URI']); ?>" id="form-confirmar-intermunicipal">
                ...
                <div class="gofast-btn-group">
                    ...
                    <a href="<?php echo esc_url(home_url('/admin-cotizar-intermunicipal')); ?>" 
...
add_shortcode('gofast_admin_solicitar_intermunicipal', 'gofast_admin_solicitar_intermunicipal_shortcode');
...
add_action('template_redirect', function() {
    ...
    if (strpos($current_url, '/admin-solicitar-intermunicipal') === false) {
        return;
    }
    ...
    // Validaciones
    if (empty($nombre) || empty($telefono) || empty($direccion_destino) || empty($destino) || $valor_envio <= 0 || empty($mensajero_id)) {
