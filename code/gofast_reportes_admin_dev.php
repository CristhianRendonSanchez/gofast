 ...
 * Shortcode: [gofast_reportes_admin_dev]
 * URL: /admin-reportes
 ***************************************************/
function gofast_reportes_admin_dev_shortcode() {
    ...
    /* ==========================================================
       5.1. Total Pagado en el Día/Rango (respeta filtros de fecha y mensajero)
    ========================================================== */
    // Usar las mismas fechas que se aplican en el resto del reporte
    $fecha_desde_pagos = $desde; // Ya viene definido arriba con el filtro o día actual
    $fecha_hasta_pagos = $hasta; // Ya viene definido arriba con el filtro o día actual
    
    // Pagos en efectivo (de la tabla pagos_mensajeros_gofast) - respeta filtros de fecha y mensajero
    $where_pagos_efectivo = "tipo_pago = 'efectivo'";
    $params_pagos_efectivo = [];
    
    // Aplicar filtro de fecha
    if ($fecha_desde_pagos !== '') {
        $where_pagos_efectivo .= " AND fecha >= %s";
        $params_pagos_efectivo[] = $fecha_desde_pagos;
    }
    if ($fecha_hasta_pagos !== '') {
        $where_pagos_efectivo .= " AND fecha <= %s";
        $params_pagos_efectivo[] = $fecha_hasta_pagos;
    }
    
    // Aplicar filtro de mensajero
    if (!$es_admin) {
        $where_pagos_efectivo .= " AND mensajero_id = %d";
        $params_pagos_efectivo[] = $usuario->id;
    } elseif ($mensajero_id > 0) {
        $where_pagos_efectivo .= " AND mensajero_id = %d";
        $params_pagos_efectivo[] = $mensajero_id;
    }
    
    $total_pagos_efectivo = (float) ($wpdb->get_var(
        $wpdb->prepare(
            "SELECT COALESCE(SUM(total_a_pagar), 0) FROM pagos_mensajeros_gofast WHERE $where_pagos_efectivo",
            $params_pagos_efectivo
        )
    ) ?? 0);
    
    // Solo transferencias de tipo 'pago' (pagos realizados por transferencia) - respeta filtros de fecha y mensajero
    $where_transferencias_pago = "tipo = 'pago' AND estado = 'aprobada'";
    $params_transferencias_pago = [];
    
    // Aplicar filtro de fecha
    if ($fecha_desde_pagos !== '') {
        $where_transferencias_pago .= " AND fecha_creacion >= %s";
        $params_transferencias_pago[] = $fecha_desde_pagos . ' 00:00:00';
    }
    if ($fecha_hasta_pagos !== '') {
        $where_transferencias_pago .= " AND fecha_creacion <= %s";
        $params_transferencias_pago[] = $fecha_hasta_pagos . ' 23:59:59';
    }
    
    // Aplicar filtro de mensajero
    if (!$es_admin) {
        $where_transferencias_pago .= " AND mensajero_id = %d";
        $params_transferencias_pago[] = $usuario->id;
    } elseif ($mensajero_id > 0) {
        $where_transferencias_pago .= " AND mensajero_id = %d";
        $params_transferencias_pago[] = $mensajero_id;
    }
    
    $total_pagos_transferencia = (float) ($wpdb->get_var(
        $wpdb->prepare(
            "SELECT COALESCE(SUM(valor), 0) FROM transferencias_gofast WHERE $where_transferencias_pago",
            $params_transferencias_pago
        )
    ) ?? 0);
    
    // Calcular total de saldos pagados: efectivo + transferencias de tipo 'pago'
    $total_pagos_dia = $total_pagos_efectivo + $total_pagos_transferencia;
...
<div class="gofast-home">
    ...
    <div class="gofast-dashboard-stats" style="display:grid;grid-template-columns:repeat(auto-fit, minmax(200px, 1fr));gap:16px;margin:24px 0;">
        ...
        <div class="gofast-box" style="text-align:center;padding:20px;background:linear-gradient(135deg, #4CAF50 0%, #45a049 100%);color:#fff;border:none;">
            <div style="font-size:18px;font-weight:600;margin-bottom:12px;color:#fff;">Saldos Pagados</div>
            <div style="font-size:32px;font-weight:700;color:#fff;margin-bottom:16px;">$<?= number_format($total_pagos_dia, 0, ',', '.'); ?></div>
            <div style="border-top:1px solid rgba(255,255,255,0.3);padding-top:12px;margin-top:12px;">
                <div style="font-size:13px;color:rgba(255,255,255,0.9);margin-bottom:6px;text-align:left;">Detalle:</div>
                <div style="font-size:12px;color:rgba(255,255,255,0.8);line-height:1.6;text-align:left;">
                    <div>💵 Efectivo - 💸 Transferencia</div>
                    <div style="font-weight:600;margin-top:4px;">
                        $<?= number_format($total_pagos_efectivo, 0, ',', '.'); ?> - $<?= number_format($total_pagos_transferencia, 0, ',', '.'); ?>
                    </div>
                </div>
            </div>
        </div>
...
add_shortcode('gofast_reportes_admin_dev', 'gofast_reportes_admin_dev_shortcode');
?>
