 ...
 * Shortcode: [gofast_transferencias_dev]
 * URL: /transferencias
 * 
 * Funcionalidades:
 ...
 ***************************************************/
function gofast_transferencias_dev_shortcode() {
    global $wpdb;
    ...
    if (isset($_POST['gofast_crear_transferencia_dev']) && wp_verify_nonce($_POST['gofast_transferencia_dev_nonce'], 'gofast_crear_transferencia_dev')) {
    ...
    if (isset($_POST['gofast_aprobar_transferencia_dev']) && wp_verify_nonce($_POST['gofast_aprobar_dev_nonce'], 'gofast_aprobar_transferencia_dev') && $rol === 'admin') {
    ...
    if (isset($_POST['gofast_rechazar_transferencia_dev']) && wp_verify_nonce($_POST['gofast_rechazar_dev_nonce'], 'gofast_rechazar_transferencia_dev') && $rol === 'admin') {
        $transferencia_id = (int) $_POST['transferencia_id'];
    ...
    if (isset($_POST['gofast_editar_transferencia_dev']) && wp_verify_nonce($_POST['gofast_editar_transferencia_dev_nonce'], 'gofast_editar_transferencia_dev') && $rol === 'admin') {
        $transferencia_id = (int) $_POST['transferencia_id'];
    ...
    if (isset($_POST['gofast_cambiar_tipo_transferencia_dev']) && wp_verify_nonce($_POST['gofast_cambiar_tipo_dev_nonce'], 'gofast_cambiar_tipo_transferencia_dev') && $rol === 'admin') {
        $transferencia_id = (int) $_POST['transferencia_id'];
    ...
    if (isset($_POST['gofast_eliminar_transferencia_dev']) && wp_verify_nonce($_POST['gofast_eliminar_dev_nonce'], 'gofast_eliminar_transferencia_dev') && $rol === 'admin') {
        $transferencia_id = (int) $_POST['transferencia_id'];
    ...
    // Solo aplicar valores por defecto si no hay ningún filtro activo
    if (empty($_GET['fecha_desde']) && empty($_GET['fecha_hasta']) && empty($_GET['estado']) && empty($_GET['tipo']) && empty($_GET['mensajero_id']) && empty($_GET['valor_min']) && empty($_GET['valor_max'])) {
        if ($rol === 'admin') {
    ...
    // Si el estado está vacío explícitamente (se seleccionó "Todos"), no aplicar filtro
    if (isset($_GET['estado']) && $_GET['estado'] === '') {
        $estado_filtro = '';
    }
    
    // Si el tipo está vacío explícitamente (se seleccionó "Todos"), no aplicar filtro
...
<div class="gofast-home">
    ...
    <div class="gofast-box" style="margin-bottom: 20px;">
        ...
        <form method="post">
            <?php wp_nonce_field('gofast_crear_transferencia_dev', 'gofast_transferencia_dev_nonce'); ?>
            
            <?php if ($rol === 'admin'): ?>
                <label><strong>Mensajero</strong> <span style="color: #dc3545;">*</span></label>
                <select name="mensajero_id" class="gofast-select" id="mensajero-select-transferencia-dev" required>
            ...
            <div class="gofast-btn-group" style="margin-top: 20px;">
                <button type="submit" name="gofast_crear_transferencia_dev" class="gofast-submit">
                                    ...
                                    <td>
                                        <div style="display: flex; gap: 5px; flex-wrap: wrap;">
                                            <form method="post" style="display: inline-block;">
                                                <?php wp_nonce_field('gofast_aprobar_transferencia_dev', 'gofast_aprobar_dev_nonce'); ?>
                                                <input type="hidden" name="transferencia_id" value="<?= esc_attr($t->id) ?>">
                                                <button type="submit" name="gofast_aprobar_transferencia_dev" 
...
<?php if ($rol === 'admin'): ?>
    <div id="modal-editar-transferencia-dev" style="display: none; position: fixed; top: 0; left: 0; right: 0; bottom: 0; background: rgba(0,0,0,0.5); z-index: 10003; overflow-y: auto; padding: 20px;">
        <div style="max-width: 600px; margin: 20px auto; background: #fff; border-radius: 8px; padding: 20px; box-shadow: 0 4px 20px rgba(0,0,0,0.3);">
            ...
            <form method="post" id="form-editar-transferencia-dev" action="">
                <?php wp_nonce_field('gofast_editar_transferencia_dev', 'gofast_editar_transferencia_dev_nonce'); ?>
                ...
                <div style="display: flex; gap: 8px; justify-content: flex-end; margin-top: 16px; padding-top: 16px; border-top: 1px solid #ddd;">
                    ...
                    <button type="submit" 
                            name="gofast_editar_transferencia_dev" 
                            class="gofast-btn-mini" 
...
<!-- Modal para rechazar transferencia (Solo Admin) -->
    <div id="modal-rechazar-transferencia-dev" style="display: none; position: fixed; top: 0; left: 0; right: 0; bottom: 0; background: rgba(0,0,0,0.5); z-index: 10000; overflow-y: auto; padding: 20px;">
        <div style="max-width: 600px; margin: 20px auto; background: #fff; border-radius: 8px; padding: 20px; box-shadow: 0 4px 20px rgba(0,0,0,0.3);">
            ...
            <form method="post" id="form-rechazar-transferencia-dev" action="">
                <?php wp_nonce_field('gofast_rechazar_transferencia_dev', 'gofast_rechazar_dev_nonce'); ?>
                ...
                <div style="display: flex; gap: 8px; justify-content: flex-end; margin-top: 16px; padding-top: 16px; border-top: 1px solid #ddd;">
                    ...
                    <button type="submit" 
                            name="gofast_rechazar_transferencia_dev" 
                            class="gofast-btn-mini" 
    ...
    <div id="modal-cambiar-tipo-transferencia-dev" style="display: none; position: fixed; top: 0; left: 0; right: 0; bottom: 0; background: rgba(0,0,0,0.5); z-index: 10002; overflow-y: auto; padding: 20px;">
        <div style="max-width: 600px; margin: 20px auto; background: #fff; border-radius: 8px; padding: 20px; box-shadow: 0 4px 20px rgba(0,0,0,0.3);">
            ...
            <form method="post" id="form-cambiar-tipo-transferencia-dev" action="">
                <?php wp_nonce_field('gofast_cambiar_tipo_transferencia_dev', 'gofast_cambiar_tipo_dev_nonce'); ?>
                ...
                <div style="display: flex; gap: 8px; justify-content: flex-end; margin-top: 16px; padding-top: 16px; border-top: 1px solid #ddd;">
                    ...
                    <button type="submit" 
                            name="gofast_cambiar_tipo_transferencia_dev" 
                            class="gofast-btn-mini" 
    ...
    <div id="modal-eliminar-transferencia-dev" style="display: none; position: fixed; top: 0; left: 0; right: 0; bottom: 0; background: rgba(0,0,0,0.5); z-index: 10001; overflow-y: auto; padding: 20px;">
        <div style="max-width: 600px; margin: 20px auto; background: #fff; border-radius: 8px; padding: 20px; box-shadow: 0 4px 20px rgba(0,0,0,0.3);">
            ...
            <form method="post" id="form-eliminar-transferencia-dev" action="">
                <?php wp_nonce_field('gofast_eliminar_transferencia_dev', 'gofast_eliminar_dev_nonce'); ?>
                ...
                <div style="display: flex; gap: 8px; justify-content: flex-end; margin-top: 16px; padding-top: 16px; border-top: 1px solid #ddd;">
                    ...
                    <button type="submit" 
                            name="gofast_eliminar_transferencia_dev" 
                            class="gofast-btn-mini" 
    ...
    function mostrarModalEditarTransferencia(id, mensajero, valor, fecha, estado) {
        ...
        document.getElementById('modal-editar-transferencia-dev').style.display = 'block';
    }
    
    function cerrarModalEditarTransferencia() {
        document.getElementById('modal-editar-transferencia-dev').style.display = 'none';
        document.getElementById('form-editar-transferencia-dev').reset();
    }
    ...
    function mostrarModalRechazar(id, mensajero, valor) {
        ...
        document.getElementById('modal-rechazar-transferencia-dev').style.display = 'block';
    }
    
    function cerrarModalRechazar() {
        document.getElementById('modal-rechazar-transferencia-dev').style.display = 'none';
        document.getElementById('form-rechazar-transferencia-dev').reset();
    }
    ...
    function mostrarModalCambiarTipo(id, mensajero, valor, tipoActual) {
        ...
        document.getElementById('modal-cambiar-tipo-transferencia-dev').style.display = 'block';
    }
    
    function cerrarModalCambiarTipo() {
        document.getElementById('modal-cambiar-tipo-transferencia-dev').style.display = 'none';
        document.getElementById('form-cambiar-tipo-transferencia-dev').reset();
    }
    ...
    function mostrarModalEliminar(id, mensajero, valor) {
        ...
        document.getElementById('modal-eliminar-transferencia-dev').style.display = 'block';
    }
    
    function cerrarModalEliminar() {
        document.getElementById('modal-eliminar-transferencia-dev').style.display = 'none';
        document.getElementById('form-eliminar-transferencia-dev').reset();
    }
    
    // Cerrar al hacer clic fuera del modal de editar
    document.getElementById('modal-editar-transferencia-dev').addEventListener('click', function(e) {
        if (e.target === this) {
    ...
    document.getElementById('modal-rechazar-transferencia-dev').addEventListener('click', function(e) {
        if (e.target === this) {
            cerrarModalRechazar();
        }
    });
    ...
    document.getElementById('modal-cambiar-tipo-transferencia-dev').addEventListener('click', function(e) {
        if (e.target === this) {
    ...
    document.getElementById('modal-eliminar-transferencia-dev').addEventListener('click', function(e) {
        if (e.target === this) {
            cerrarModalEliminar();
        }
    });
...
add_shortcode('gofast_transferencias_dev', 'gofast_transferencias_dev_shortcode');
