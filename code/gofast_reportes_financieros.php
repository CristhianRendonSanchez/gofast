...
function gofast_reportes_financieros_shortcode() {
    ...
    $where_ingresos = "tracking_estado != 'cancelado'" . str_replace('fecha', 's.fecha', $where_fecha);
    $total_ingresos_servicios = (float) $wpdb->get_var(
        !empty($params_fecha) 
            ? $wpdb->prepare("SELECT COALESCE(SUM(total), 0) FROM servicios_gofast s WHERE tracking_estado != 'cancelado' AND DATE(fecha) >= %s AND DATE(fecha) <= %s", $fecha_desde, $fecha_hasta)
    ...
    $total_ingresos_compras = (float) $wpdb->get_var(
        !empty($params_fecha)
            ? $wpdb->prepare("SELECT COALESCE(SUM(valor), 0) FROM compras_gofast WHERE estado != 'cancelada' AND DATE(fecha_creacion) >= %s AND DATE(fecha_creacion) <= %s", $fecha_desde, $fecha_hasta)
    ...
    $total_egresos = (float) $wpdb->get_var(
        !empty($params_fecha)
            ? $wpdb->prepare("SELECT COALESCE(SUM(valor), 0) FROM egresos_gofast WHERE DATE(fecha) >= %s AND DATE(fecha) <= %s", $fecha_desde, $fecha_hasta)
    ...
    $total_vales_empresa = (float) $wpdb->get_var(
        !empty($params_fecha)
            ? $wpdb->prepare("SELECT COALESCE(SUM(valor), 0) FROM vales_empresa_gofast WHERE DATE(fecha) >= %s AND DATE(fecha) <= %s", $fecha_desde, $fecha_hasta)
    ...
    $total_transferencias_ingresos = (float) $wpdb->get_var(
        !empty($params_fecha)
            ? $wpdb->prepare("SELECT COALESCE(SUM(valor), 0) FROM transferencias_gofast WHERE estado = 'aprobada' AND DATE(fecha_creacion) >= %s AND DATE(fecha_creacion) <= %s", $fecha_desde, $fecha_hasta)
    ...
    $total_transferencias_salidas = (float) $wpdb->get_var(
        !empty($params_fecha)
            ? $wpdb->prepare("SELECT COALESCE(SUM(valor), 0) FROM transferencias_salidas_gofast WHERE DATE(fecha) >= %s AND DATE(fecha) <= %s", $fecha_desde, $fecha_hasta)
    ...
    $total_descuentos = (float) $wpdb->get_var(
        !empty($params_fecha)
            ? $wpdb->prepare("SELECT COALESCE(SUM(valor), 0) FROM descuentos_mensajeros_gofast WHERE DATE(fecha) >= %s AND DATE(fecha) <= %s", $fecha_desde, $fecha_hasta)
    ...
    foreach ($mensajeros as $mensajero) {
        ...
        $total_a_pagar = $comision_generada - $transferencias_aprobadas - $descuentos - $pagos_rango;
    ...
    $total_saldos_pendientes = array_sum(array_map(function($s) { return max(0, $s->total_a_pagar); }, $saldos_mensajeros));
...
<div class="container">
    ...
    <div class="summary-grid">
        <div class="summary-box">
            <div class="value">$' . number_format($total_ingresos, 0, ',', '.') . '</div>
            <div class="label">Total Ingresos</div>
        </div>
        <div class="summary-box">
            <div class="value">$' . number_format($total_comisiones, 0, ',', '.') . '</div>
            <div class="label">Comisiones (20%)</div>
        </div>
        <div class="summary-box">
    ...
    <div class="section">
        ...
        <table>
            <tr><td>💰 Total Ingresos (Servicios + Compras)</td><td class="text-right">$' . number_format($total_ingresos, 0, ',', '.') . '</td></tr>
            <tr><td>💵 Comisiones (20% de Ingresos)</td><td class="text-right">$' . number_format($total_comisiones, 0, ',', '.') . '</td></tr>
...
<body>
    ...
    if (isset($_POST['gofast_enviar_reporte']) && wp_verify_nonce($_POST['gofast_enviar_reporte_nonce'], 'gofast_enviar_reporte')) {
        ...
        if (empty($email_destino) || !is_email($email_destino)) {
            ...
            // Redirigir para generar y enviar
            $url_reporte = add_query_arg([
                'fecha_desde' => $fecha_desde,
                'fecha_hasta' => $fecha_hasta,
                'tipo' => $tipo_reporte_email,
                'accion' => 'ver'
            ], home_url('/reportes-financieros'));
            
            // Generar el reporte HTML para el correo (reusar la lógica de arriba)
            // Por ahora, enviar un correo simple
            $headers = array('Content-Type: text/html; charset=UTF-8');
            $cuerpo = "Se ha solicitado un reporte financiero. Accede al siguiente enlace para verlo:<br><br><a href='{$url_reporte}'>{$url_reporte}</a>";
            
            if (wp_mail($email_destino, $asunto_reporte, $cuerpo, $headers)) {
                $mensaje = '✅ Enlace del reporte enviado a ' . esc_html($email_destino);
...
<head>
    ...
    <style>
        * { margin: 0; padding: 0; box-sizing: border-box; }
        ...
        .report-card { border: 2px solid #eee; border-radius: 8px; padding: 20px; }
        .report-card h4 { margin-top: 0; }
        .report-card p { color: #666; font-size: 13px; margin-bottom: 16px; }
        .btn-group { display: flex; gap: 8px; flex-wrap: wrap; }
        .alert { padding: 12px 16px; border-radius: 6px; margin-bottom: 16px; }
        .alert-success { background: #d4edda; color: #155724; }
        .alert-error { background: #f8d7da; color: #721c24; }
        .back-link { display: inline-block; margin-bottom: 16px; color: #666; text-decoration: none; }
